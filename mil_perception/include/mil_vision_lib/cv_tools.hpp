...
namespace mil_vision {
     ...
     * @param param Ros param in the global namespace to load points from
     ...
     */
    bool get3DPointsFromParam(std::string param, std::vector<cv::Point3f>& points);
    /// @param value A ROS XML value for parsing points from an already resolved param
    bool get3DPointsFromXml(XmlRpc::XmlRpcValue& value, std::vector<cv::Point3f>& points);
    /// @param nh Ros nodehandle to retrieve topic from
    /// @param param Ros param in nh's namespace to load points from
    bool get3DPointsFromParam(ros::NodeHandle& nh, std::string param, std::vector<cv::Point3f>& points);
